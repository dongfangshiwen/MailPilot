...
class Migration180Test {
    @Test fun versionSevenRetainsCredentialsHistoryAndAddsDiagnosticIdentity()=runBlocking {
        val context: Application=RuntimeEnvironment.getApplication(); val name="migration180-${System.nanoTime()}.db"
        val path=context.getDatabasePath(name); path.parentFile!!.mkdirs()
        val schema=JSONObject(File(System.getProperty("mailpilot.schemas"),"app.mailpilot.data.MailDatabase/7.json").readText()).getJSONObject("database")
        SQLiteDatabase.openOrCreateDatabase(path,null).use { sqlite ->
            ...
            insert("models",mapOf("id" to "model","label" to "旧配置","model" to "custom-text","baseUrl" to "https://proxy.invalid/v1","apiKeyCipher" to "encrypted-key","supportsVision" to 0))
        ...
        try {
            ...
            try {
                ...
                assertEquals("complete",db.dao().history("c").single().resultStatus)
                assertEquals("{}",db.dao().history("c").single().failureJson)
                ...
                assertEquals("",db.dao().history("c").single().action)
