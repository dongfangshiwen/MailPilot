...
class Migration170Test {
    @Test fun versionSixPreservesRecordsAndAddsLocalMaterialsAndOptions()=runBlocking {
        val context: Application=RuntimeEnvironment.getApplication(); val name="migration170-${System.nanoTime()}.db"
        val path=context.getDatabasePath(name); path.parentFile!!.mkdirs()
        val schema=JSONObject(File(System.getProperty("mailpilot.schemas"),"app.mailpilot.data.MailDatabase/6.json").readText()).getJSONObject("database")
